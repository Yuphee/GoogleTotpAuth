...
public class TotpCountdownTask implements Runnable {
  ...
  private long getCounterValue(long time) {
    return CountUtils.getValueAtTime(Utilities.millisToSeconds(time));
  }
  ...
  private long getTimeTillNextCounterValue(long time) {
    ...
    long nextValueStartTime = Utilities.secondsToMillis(CountUtils.getValueStartTime(nextValue));
  ...
  private long getCounterValueAge(long time) {
    return time - Utilities.secondsToMillis(CountUtils.getValueStartTime(getCounterValue(time)));
  }
}
